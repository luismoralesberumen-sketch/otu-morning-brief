...
Runs on Render 24/7, fires briefs Mon-Fri at 9:30, 11:30, 1:30, 3:30 PM ET
Data: Charles Schwab Trader API (options, prices, history)
      Yahoo Finance (VIX only — 1 call)
"""

import os, time, base64, json, statistics, datetime, threading
...
def get_schwab_headers():
    ...
    try:
        ...
        if "access_token" in data:
            ...
            print(f"  Schwab token refreshed OK")
        else:
            print(f"  Schwab token refresh failed: {data.get('error')}")
...
def get_macro(schwab_headers):
    try:
        ...
        if vix_price < 10:   cash_pct, deploy_pct = 87, 13
        elif vix_price < 15: cash_pct, deploy_pct = 62, 38
        elif vix_price < 20: cash_pct, deploy_pct = 37, 63
        elif vix_price < 30: cash_pct, deploy_pct = 0, 100
        else:                cash_pct, deploy_pct = 0, 100

        print(f"  VIX={vix_price:.2f} SPY=${spy_price:.2f} EMA200=${ema200:.2f}")
        return {
            "vix": round(vix_price, 2), "spy": round(spy_price, 2), "ema200": ema200,
            "cash_pct": cash_pct, "deploy_pct": deploy_pct, "bear_market": spy_price < ema200
        }
    except Exception as e:
...
def get_options_data(schwab_headers, ticker):
    try:
        ...
        # Find expiry closest to TARGET_EXPIRY (min 7 DTE)
        ...
        best_contract, best_delta_diff = None, 999
        for strike_str, contracts in strikes_data.items():
            ...
            if diff < best_delta_diff:
                best_delta_diff, best_contract = diff, (float(strike_str), c)
...
def run_brief(slot_label):
    ...
    print(f"\n{'='*60}\nBrief: {slot_label} ET -- {now_et.strftime('%Y-%m-%d %H:%M')}\n{'='*60}")

    schwab_headers = get_schwab_headers()
    if not schwab_headers:
        send_discord(f"**OTU Brief {slot_label} ET** — ERROR: Schwab token unavailable. Re-auth required.")
        return
    ...
    if qualified:
        ...
        # Detect if Schwab is returning data at all
        if len(skipped) == len(TICKERS):
            send_discord(
                f"**OTU Brief -- {now_et.strftime('%a %b %d, %Y')} | {slot_label} ET**\n\n"
                f"ERROR: Schwab API returned no data for any ticker. Token may need re-auth.\n"
                f"Visit: https://api.schwabapi.com/v1/oauth/authorize?response_type=code"
                f"&client_id={SCHWAB_CLIENT_ID}&redirect_uri=https%3A%2F%2F127.0.0.1"
            )
            return
...
def main():
    ...
    print(f"Schwab configured: {'YES' if SCHWAB_CLIENT_ID else 'NO'}")
    ...
    print("Scheduler running. Waiting for next trigger...\n")
    try:
        while True:
            time.sleep(60)
